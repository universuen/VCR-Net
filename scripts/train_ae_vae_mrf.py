...
# MRF energy function
def mrf_loss(dehazed_image):
    def pairwise_potential(x):
        return torch.sum(torch.abs(x[:, :, :-1] - x[:, :, 1:])) + torch.sum(torch.abs(x[:, :-1, :] - x[:, 1:, :]))
    mrf_term = pairwise_potential(dehazed_image)
    return mrf_term
...
for e in range(config.Training.epochs):
    ...
    for idx, (clear_imgs, hazy_imgs) in enumerate(tr_dataloader):
        ...
        # loss_mrf = mrf_loss(dehazed_imgs)
        ...
        loss = reconstruct_loss + config.Training.alpha * contrastive_loss \
                + config.Training.beta * kl_divergence \
                # + config.Training.gamma * loss_mrf
        ...
        avg_contra_loss += config.Training.alpha * contrastive_loss 
        # avg_mrf_loss += config.Training.gamma * loss_mrf
    ...
    logger.info(
        ...
        # f'avg_mrf_loss: {avg_mrf_loss:.4f}, '
    )
